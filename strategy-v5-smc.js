...
// ═══════════════════════════════════════════════════════════════
//  strategy-v5-smc.js  —  4-Token SMC — All Symbols Concurrent
//
//  V5 changes from V4:
//    • All 4 tokens trade concurrently — BTC, ETH, SOL, BNB
//    • BTC: 75× leverage (tighter SL suits lower volatility)
//    • ETH / SOL / BNB: 20× leverage (wider SL for higher vol)
//    • Optimal params from 30-day param sweep (150 combos × 2 symbols)
//    • No cross-symbol blocking — each token independent
//
...
// ── V5: 4-token concurrent config — optimised from 30-day param sweep ──
// All 4 tokens trade at the same time — no cross-symbol blocking.
// BTC uses 75× (lower vol → tighter SL fine).
// ETH/SOL/BNB use 20× (higher vol → wider SL avoids whipsaw stops).
const ACTIVE_SYMBOLS = ['BTCUSDT', 'ETHUSDT', 'SOLUSDT', 'BNBUSDT'];
...
// SL price distance = 0.50% capital / leverage
const SYMBOL_SL_PCT = {
  BTCUSDT: 0.50 / 75,  // 0.0067% price — BTC
  ETHUSDT: 0.50 / 20,  // 0.025%  price — ETH
  SOLUSDT: 0.50 / 20,  // 0.025%  price — SOL
  BNBUSDT: 0.50 / 20,  // 0.025%  price — BNB
};
