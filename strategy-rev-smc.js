...
// ═══════════════════════════════════════════════════════════════
//  strategy-rev-smc.js — 5m Structure-Reversal SMC Scanner
//
//  Entry: VWAP zone + 5m/15m/4H pivot confluence (same filter stack
//  as master-grid winner: 100x SL20% Rev, vol1.0, lb5/10, cdY, chY).
//  Exit: handled by cycle.js via shouldExitRev (5m pivot reversal).
//
//  Symbols: BNBUSDT, SOLUSDT (and any non-BTC/ETH alts).
...
const SYMBOL_SL_PCT   = { BNBUSDT: 0.0020, SOLUSDT: 0.0020 }; // 0.20% price = 20% capital @ 100x
...
function getHommaSignal() { return { bias: 'NEUTRAL', score: 0, patterns: [] }; }

function resolveSignal(st, zone, price, vwap) {
  const p5m = st.last5mPivotType;
  const sideways = isSidewaysMarket(st);
  const pMacro = st.last15mPivotType;
  const getMacro = (s, cp) => getMacroStructure(s, cp, sideways);
  function _rej() { return null; }

  function tryLong() {
    const s4h = get4hStructure(st, price);
    const sMacro = getMacro(st, price);
    if (s4h === 'BEARISH') return _rej();
    if ((s4h === 'MIXED' || s4h === 'UNKNOWN') && sMacro === 'BEARISH') return _rej();
    const isDemandMacro = sideways ? pMacro === 'HL' : pMacro === 'LL' || pMacro === 'HL';
    const isDemand5m = p5m === 'LL' || p5m === 'HL';
    if (!isDemandMacro || !isDemand5m) return _rej();
    const isTrendCont = !sideways && s4h === 'BULLISH' && pMacro === 'HL' && p5m === 'HL';
    if (!isTrendCont) {
      const homma = getHommaSignal();
      if (homma.bias === 'BEARISH' && homma.score >= 2) return _rej();
      if (!checkVolumeSpike(st.candles5m)) return _rej();
    }
    if (isChasing(price, st.sl5m_1, st.symbol)) return _rej();
    if (vwap && vwap.stddev > 0) {
      if ((price - vwap.upper) / vwap.stddev > MIN_DIST_SIGMA * 2) return _rej();
    }
    return { direction: 'LONG', type: `${pMacro}+${p5m}`, regime: sideways ? 'SIDEWAYS' : 'TRENDING' };
  }

  function tryShort() {
    const s4h = get4hStructure(st, price);
    const sMacro = getMacro(st, price);
    if (s4h === 'BULLISH') return _rej();
    if ((s4h === 'MIXED' || s4h === 'UNKNOWN') && sMacro === 'BULLISH') return _rej();
    const isSupplyMacro = sideways ? pMacro === 'LH' : pMacro === 'HH' || pMacro === 'LH';
    const isSupply5m = p5m === 'HH' || p5m === 'LH';
    if (!isSupplyMacro || !isSupply5m) return _rej();
    const isTrendCont = !sideways && s4h === 'BEARISH' && pMacro === 'LH' && p5m === 'LH';
    if (!isTrendCont) {
      const homma = getHommaSignal();
      if (homma.bias === 'BULLISH' && homma.score >= 2) return _rej();
      if (!checkVolumeSpike(st.candles5m)) return _rej();
    }
    if (isShortTooLate(price, st.last15mPivotPrice, st.symbol)) return _rej();
    if (isShort5mTooLate(price, st.sh5m_1, st.symbol)) return _rej();
    if (vwap && vwap.stddev > 0) {
      if ((vwap.lower - price) / vwap.stddev > MIN_DIST_SIGMA * 2) return _rej();
    }
    return { direction: 'SHORT', type: `${pMacro}+${p5m}`, regime: sideways ? 'SIDEWAYS' : 'TRENDING' };
  }

  const isLowPivot  = sideways ? pMacro === 'HL' : pMacro === 'LL' || pMacro === 'HL';
  const isHighPivot = sideways ? pMacro === 'LH' : pMacro === 'HH' || pMacro === 'LH';
  if ((zone === 'LOWER_MID' || zone === 'BELOW_LOWER') && isLowPivot)  return tryLong();
  if ((zone === 'UPPER_MID'  || zone === 'ABOVE_UPPER') && isHighPivot) return tryShort();
  return null;
}
...
async function fetchAndUpdate(symbol, log) {
  ...
  if (!st.ready) {
    ...
    // Seed closed bars (drop last live bar)
    ...
    st.ready = true;
    log(`[Rev] ${symbol} ready | 15m=${st.last15mPivotType||'none'} 5m=${st.last5mPivotType||'none'} 4H=${get4hStructure(st, null)}`);
    return;
  }

  // Incremental: fetch latest delta candles
...
async function analyzeRev(symbol, log = console.log) {
  const st = getState(symbol);
  ...
  if (st.pendingSignal) {
    ...
    if (vwapNext) {
      ...
      if (resolved && resolved.direction === st.pendingSignal.direction && frozenOk) {
        ...
        log(`[Rev] ✓ ${symbol} ${signal.direction} entry=$${entryPrice.toFixed(4)} zone=${zoneNext} type=${signal.type}`);
      } else {
        log(`[Rev] pending ${symbol} ${st.pendingSignal.direction} CANCELLED`);
      }
    }
  ...
  if (pivotResult && pivotResult.time !== st.lastSignalTime) {
    ...
    if (vwap) {
      ...
      if (sig && !st.zoneTraded) {
        ...
        log(`[Rev] pivot → pending ${symbol} ${sig.direction} zone=${zone} type=${sig.type} — next candle open`);
      }
    }
  }
...
// ── Multi-symbol scan ──────────────────────────────────────────
async function scanRevSMC(log = console.log) {
  const results = [];
  for (const sym of ACTIVE_SYMBOLS) {
    try {
      const sig = await analyzeRev(sym, log);
      if (sig) results.push(sig);
      await new Promise(r => setTimeout(r, 200));
    } catch (e) {
      log(`[Rev] ${sym} error: ${e.message}`);
    }
  }
  log(`[Rev] scan done — ${results.length} signal(s)`);
  return results;
}
